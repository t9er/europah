...
def field(theta, phi):
    ...
    E_glob = np.zeros((1000, 1000), dtype=complex)
    ...
    for l in range(2, 78, 1):
        ...
        #integrate Er over radius to get Er_surf

        Er_surf = np.trapz(Er[::-1], x = dyn.cheby.xi[::-1]/np.pi*dyn.Rp)

        # obtain E_glob by multiplying Er_surf with Y_lm.

        # Er_grd, th_grd = np.meshgrid(Er, theta)
        th_grd, ph_grd = np.meshgrid(theta, phi)

        sph = SH(2, l, ph_grd, th_grd)    
        
        E_glob += sph*Er_surf

        y_l +=1

       # field_grd += (Er_grd*SH(2, l, 0, th_grd))
    ...
    for i in range(1, 1000):
       field_grd_final[i] = np.trapz(field_grd[i, :], x = (dyn.Rp-dyn.cheby.xi/np.pi*dyn.Rp)/1e5)

    for r in range(0, 1000, 1):
        for c in range(0, 1000, 1):
            sph[c, r] =  sph[c, r] * field_grd_final[r]

    plt.contourf(ph_grd, th_grd, E_glob, levels=300,  cmap='inferno', extend='min')
    plt.show()
